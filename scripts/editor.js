...
export class AuraEditor extends FormApplication {
    ...
    activateListeners(html) {
        ...
        // Unwrap jQuery-wrapped html to native Element (Foundry may pass a jQuery object)
        const root = html instanceof HTMLElement ? html : html[0];

        // 处理动画选择变化（使用原生事件与选择器）
        // Handle animation selection changes using native events and selectors
        ...
        // 处理条件模式变化
        // Handle changes to the condition mode (script vs simple)
        ...
        // Update range slider fill visuals when value changes (fixes missing fill bar behavior)
        ...
        // 初始化动画选项可见性（加载时）
        // Initialize animation option visibility on load
        ...
        // 初始化条件模式可见性（加载时）
        // Initialize condition mode visibility on load
        ...
        // 初始化范围滑块显示（加载时）
        // Initialize range slider display on load
        root.querySelectorAll('input[type="range"]').forEach(slider => {
            const valueSpan = slider.parentElement.querySelector('.range-value');
            if (valueSpan) {
                let unit = '';
